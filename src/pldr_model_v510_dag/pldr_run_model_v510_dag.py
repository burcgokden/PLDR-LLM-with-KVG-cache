'''
LLM from Power Law Decoder Representations v510_dag (PLDR-LLM v510_dag) with KV-cache and G-cache
Train and Evaluation Module
'''
...
from pldr_model_v510_dag import (plgMultiHeadAttention, PLDR_DecoderLayer, PLDR_Model,
                                 PLDR_Decoder, GLUVariant, ResLayerA)
from power_law_attention_layer_v510_dag import plga_layer

import pldr_model_v510_dag as pldr_model
import common as cm
...
class dist_pldr_model_e2e:
    ...
    def __init__(self, rank, world_size, inp_obj_src, hpdict=None,
        ...
        if is_train or os.path.isdir(load_ckpt if load_ckpt is not None else ""):
            ...
            #train parameters
            assert all([c>=0 for c in self.hpdict["DAG_reg_coeffs"]]) and isinstance(self.hpdict["DAG_reg_coeffs"], list), \
                                   f"Invalid DAG regularization coefficients: {self.hpdict['DAG_reg_coeffs']}"
            
            self.DAG_reg_coeffs=hpdict["DAG_reg_coeffs"]           
            self.loss_function=masked_loss_dagreg
    ...
    def dist_train_step(self, inp):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            ...
            predictions, _, att_weights, _ = self.dist_pldr_model([tar_inp, combined_mask], kvcachelst=None, Gcachelst=None)
            loss, dagA, dagAp, dagG=self.loss_function(tar_real, predictions, att_weights, self.DAG_reg_coeffs, device=self.device)
        ...
        with torch.no_grad():
            ...
            self.train_daga.update_state(dagA.item())
            self.train_dagap.update_state(dagAp.item())
            self.train_dagg.update_state(dagG.item())
    ...
    def dist_train_model(self, train_batches, train_batches_cnt=None, 
        ...
        self.train_daga=keras.metrics.Mean(name='train_loss_a')
        self.train_dagap=keras.metrics.Mean(name='train_loss_ap')
        self.train_dagg=keras.metrics.Mean(name='train_loss_g')
        self.val_daga=keras.metrics.Mean(name='val_loss_a')
        self.val_dagap=keras.metrics.Mean(name='val_loss_ap')
        self.val_dagg=keras.metrics.Mean(name='val_loss_g')
        ddp_trndag_res= torch.zeros(3).to(self.device)
        ddp_valdag_res= torch.zeros(3).to(self.device)
        ...
        trn_daga_lst, trn_dagap_lst, trn_dagg_lst=[],[],[]
        val_daga_lst, val_dagap_lst, val_dagg_lst=[],[],[]
            ...
            for batch in range(train_batches_cnt):
                ...
                with torch.no_grad():
                    if batch_cnt % verbose_freq == 0 or batch==0:                   
                        ...
                        dagA_batch=self.train_daga.result().item()
                        dagAp_batch=self.train_dagap.result().item()
                        dagG_batch=self.train_dagg.result().item()
                        ddp_trndag_res= torch.tensor([dagA_batch, dagAp_batch, dagG_batch]).to(self.device)
                        dist.all_reduce(ddp_trndag_res, op=dist.ReduceOp.SUM)
                        dagA_batch, dagAp_batch, dagG_batch=(ddp_trndag_res/self.world_size).tolist()
                        ...
                        if self.rank==0:
                            logger.info(f"{time_so_far:.2f}s Epoch {epoch + 1} Batch {batch_cnt} Loss(G) {loss_epoch:.4f} Acc(G) {acc_epoch:.4f} "+
                                ...
                                f"LR {self.scheduler.get_last_lr()[0]:.4e} DAGL A(R) {dagA_batch:.4e} DAGL Ap(R) {dagAp_batch:.4e} DAGL G(R) {dagG_batch:.4e}")
                        ...
                        trn_daga_lst.append(dagA_batch)
                        trn_dagap_lst.append(dagAp_batch)
                        trn_dagg_lst.append(dagG_batch)

                        self.train_loss.reset_state()
                        self.train_accuracy.reset_state()
                        self.train_daga.reset_state()
                        self.train_dagap.reset_state()
                        self.train_dagg.reset_state()
                    ...
                    if val_verbose_freq is not None:
                        if batch_cnt % val_verbose_freq == 0:
                            ...
                            val_dagAv=self.val_daga.result().item()
                            val_dagApv=self.val_dagap.result().item()
                            val_dagGv=self.val_dagg.result().item()
                            ddp_valdag_res=torch.tensor([val_dagAv, val_dagApv, val_dagGv]).to(self.device)
                            dist.all_reduce(ddp_valdag_res, op=dist.ReduceOp.SUM)
                            val_dagAv, val_dagApv, val_dagGv=(ddp_valdag_res/self.world_size).tolist()

                            val_loss_lst.append(val_lossv)
                            val_acc_lst.append(val_accv)
                            val_daga_lst.append(val_dagAv)
                            val_dagap_lst.append(val_dagApv)
                            val_dagg_lst.append(val_dagGv)
                            if self.rank==0:
                                logger.info(f"{time_so_far:.2f}s Epoch {epoch + 1} Batch {batch_cnt} Val Loss {val_lossv:.4f} Val Accuracy {val_accv:.4f} "+
                                            f"DAGL A(R) {val_dagAv:.4e} DAGL Ap(R) {val_dagApv:.4e} DAGL G(R) {val_dagGv:.4e}")
            ...
            with torch.no_grad():
                ...
                if self.rank==0:
                    logger.info(f"Epoch {epoch + 1} Loss(G) {loss_epoch:.4f} Accuracy(G) {acc_epoch:.4f} "+
                        ...
                        f"DAGL A(R) {dagA_batch:.4e} DAGL Ap(R) {dagAp_batch:.4e} DAGL G(R) {dagG_batch:.4e}")
        ...
        if self.rank==0:
            ...
            cm.pklsave(self.train_ckpt_path+'/train_DAG_losses.pkl', [trn_daga_lst, trn_dagap_lst, trn_dagg_lst])
            cm.pklsave(self.train_ckpt_path+'/val_DAG_losses.pkl', [val_daga_lst, val_dagap_lst, val_dagg_lst])
    ...
    def validate_step(self, inp):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            predictions, _,att_weights,_ = self.dist_pldr_model([tar_inp, combined_mask], kvcachelst=None, Gcachelst=None)
            loss, dagA, dagAp, dagG=self.loss_function(tar_real, predictions, att_weights, 
                                                       self.DAG_reg_coeffs, device=self.device)
        loss_=loss.item()
        acc_=self.accuracy_function(tar_real, predictions).item()

        self.val_loss.update_state(loss_)
        self.val_accuracy.update_state(acc_)
        self.val_daga.update_state(dagA.item())
        self.val_dagap.update_state(dagAp.item())
        self.val_dagg.update_state(dagG.item())
    ...
    def generate_text(self, sentence, 
        ...
        if Gcachelst is not None:
            Gcachelst=[[AW.to(self.device), avAp.to(self.device), Ap.to(self.device)] for AW, avAp, Ap in Gcachelst]
        cached=False
        ...
        for i in range(max_length):
            ...
            if enable_Gcache:
                Gcachelst=[[t[0],t[4], t[-1]] for t in att_weights]
...
def dag_regularizer(att_weights, device):
    '''
    ...
    att_weights[0] is metric tensor, A_LM
    att_weights[-1] is potential tensor, Ap
    att_weights[4] is energy_curvature tensor, G_LM
    ...
    adjm=torch.stack([t[0] for t in att_weights], dim=0).to(device=device, dtype=torch.float32) #shape: [layer_num, batch_size, num_heads, dk, dk]
    adjp=torch.stack([t[-1] for t in att_weights], dim=0).to(device=device, dtype=torch.float32)
    adjg=torch.stack([t[4] for t in att_weights], dim=0).to(device=device, dtype=torch.float32)

    #the sum of diagonals need to be equal to dk=dmodel//num_heads
    dval=torch.tensor(adjm.size()[-1]).to(device=device, dtype=torch.float32)

    #average only on batch_size version
    adjm_dloss=torch.abs(torch.log(torch.einsum('...ii->...', (torch.linalg.matrix_exp(torch.mul(adjm, adjm))))/dval)) #shape: [layer_num, batch_size, num_head]
    adjp_dloss=torch.abs(torch.log(torch.einsum('...ii->...', (torch.linalg.matrix_exp(torch.mul(adjp, adjp))))/dval))
    adjg_dloss=torch.abs(torch.log(torch.einsum('...ii->...', (torch.linalg.matrix_exp(torch.mul(adjg, adjg))))/dval))

    adjm_dloss=torch.mean(adjm_dloss) #mean over batch_size*layer*num_head
    adjp_dloss=torch.mean(adjp_dloss)
    adjg_dloss=torch.mean(adjg_dloss)

    return adjm_dloss, adjp_dloss, adjg_dloss

def masked_loss_dagreg(y_true, y_pred, att_weights, reg_coeffs, device):
    '''
    masked loss with dag regularization
    y_true: ground truth
    y_pred: predictions
    att_weights: attention weights
    reg_coefficients: three element list of dag loss weights for A, Ap and G.
    returns loss added with dag loss of deductive outputs
    '''
    loss_=masked_loss_function(y_true, y_pred)
    if np.sum(reg_coeffs) > 0:
        dagA, dagAp, dagG=dag_regularizer(att_weights, device=device)
        cA,cAp,cG=reg_coeffs
        dagA_reg=cA*dagA if cA > 0 else 0
        dagAp_reg=cAp*dagAp if cAp > 0 else 0
        dagG_reg=cG*dagG if cG > 0 else 0
    return (loss_+ dagA_reg+dagAp_reg+dagG_reg), dagA, dagAp, dagG
